<script>
  ...
  function buildSources(cx, cy, W, H, SPEED) {
    ...
    return [
      { sx: cx, sy: cy, a: 1.0, bornAt: 0 },
      { sx: -cx, sy: cy, a: 0.7, bornAt: s(cx) },
      { sx: 2 * W - cx, sy: cy, a: 0.7, bornAt: s(W - cx) },
      { sx: cx, sy: -cy, a: 0.7, bornAt: s(cy) },
      { sx: cx, sy: 2 * H - cy, a: 0.7, bornAt: s(H - cy) },
      { sx: -cx, sy: -cy, a: 0.4, bornAt: s(Math.hypot(cx, cy)) },
      {
        sx: 2 * W - cx,
        sy: -cy,
        a: 0.4,
        bornAt: s(Math.hypot(W - cx, cy)),
      },
      {
        sx: -cx,
        sy: 2 * H - cy,
        a: 0.4,
        bornAt: s(Math.hypot(cx, H - cy)),
      },
      {
        sx: 2 * W - cx,
        sy: 2 * H - cy,
        a: 0.4,
        bornAt: s(Math.hypot(W - cx, H - cy)),
      },
    ];
  }
  ...
  function spawnWave(cx, cy, forcedVariant, bypassLimit = false) {
    ...
    const SPEED = variant === "matrix" ? 440 : variant === "vercel" ? 260 : 220;

    if (variant === "matrix") addMatrixEffect();

    const highlightEl =
      variant === "vercel"
        ? document.querySelector(".tl-vercel")
        : variant === "clerk"
          ? document.querySelector(".tl-clerk")
          : null;
  ...
  function drawWave(ctx, elapsed, variant, SPEED, sources, rainbowGrads) {
    ...
    for (let si = 0; si < sources.length; si++) {
      ...
      if (variant === "glitch") {
        ...
        ctx.strokeStyle = `rgba(255,60,60,${(alpha * 0.85).toFixed(3)})`;
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(sx + dx, sy + dy, r, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(60,255,220,${(alpha * 0.85).toFixed(3)})`;
        ctx.stroke();
        continue;
      }
      ...
      if (variant === "ripple") {
        ...
        for (const [offset, fade] of [
          ...
          ctx.strokeStyle = `rgba(255,102,0,${(alpha * fade).toFixed(3)})`;
          ctx.stroke();
        }
        continue;
      }
      ...
      if (variant === "vercel") {
        ...
        const rgb = document.documentElement.classList.contains("theme-dark")
          ? "255,255,255"
          : "0,0,0";
        ctx.beginPath();
        ...
        const layers = [
          {
            blur: 24,
            width: 12,
            color: `rgba(160,60,255,${(alpha * 0.18).toFixed(3)})`,
          },
          {
            blur: 16,
            width: 5,
            color: `rgba(190,100,255,${(alpha * 0.45).toFixed(3)})`,
          },
          {
            blur: 8,
            width: 2,
            color: `rgba(230,180,255,${(alpha * 0.9).toFixed(3)})`,
          },
        ];
  ...
  function addMatrixEffect() {
    ...
    ensureCanvas();
    if (matrixFx) {
      // restart rather than stack a second instance
  ...
  const tlTotalMonths =
    (tlNow.getFullYear() - tlStart.getFullYear()) * 12 + (tlNow.getMonth() - tlStart.getMonth());

  // Percentage from the left (now) for any date on the timeline
  function tlPct(date) {
    const months =
      (tlNow.getFullYear() - date.getFullYear()) * 12 + (tlNow.getMonth() - date.getMonth());
    ...
    <div class="my-5 py-2 animated fadeIn" style="animation-delay: 0.25s">
      ...
      <nav class="social-links" aria-label="Social media and contact links">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Schedule a time using cal.com"
        >
  ...
  >
    <div class="tl-wrap">
      <div class="tl-labels" aria-hidden="true">
        ...
        <div
          class="tl-company tl-vercel"
          data-tenure="Feb '26 ← Nov '18"
          style="left: {vercelLabelLeft}%"
        >
          <svg viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
